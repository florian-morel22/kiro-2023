...
def v_C_n(z_cables,substations,scenario):
    ...
    for sp,substation in enumerate(substations):
        ...
        for z_cable in z_cables:
            z_id+=1
            c_n+=wind_scenarios[scenario]["power_generation"]*(z_cable["s_id"]==sp)
                #construction cable turbine
        
        c_n1=0
        c_n2=0
        for type_s in range(nb_s_type):
            c_n1+=(substation["type_s"]!=nb_s)*(substation["type_linked_s"]==type_s)*s_type[type_s]["rating"]
        
        for type_s_s in range(nb_s_s_cables):
            for s in range(nb_s):
                c_n2+=(substation["type_s"]!=nb_s)*(substation["type_linked_s"]==type_s_s)*(substation["linked_s"]==s)*(s_s_cables[type_s_s]["rating"])
        c_n-=min(c_n1,c_n2)
...
def cplexsolve():
    ...
    for sp,substation in enumerate(substations):
        ...
        for type_s_s in range(nb_s_s_cables):
            for s in range(nb_s):
                c_cost+=0.5*(substation["type_s"]!=nb_s)*(substation["type_linked_s"]==type_s_s)*(substation["linked_s"]==s)*(s_s_cables[type_s_s]["fixed_cost"]+s_s_cables[type_s_s]["variable_cost"]*np.sqrt((s_loc[sp]["x"]-s_loc[s]["x"])**2+(s_loc[sp]["y"]-s_loc[s]["y"])**2))
    z_id=-1
    for z_cable in z_cables:
        z_id+=1
        for k in range(nb_s):
    ...
    for scenario in range(nb_wind_scenarios):
        for sp,substation in enumerate(substations):
            for s in range(nb_s_type): 
                op_cost_thomas-=(substation["type_s"]!=nb_s)*(substation["type_s"]==s)*s_type[s]["probability_of_failure"]
        c_cost+=op_cost_thomas  *param["curtailing_cost"] *v_C_n(z_cables,substations,scenario)  
